    ...
    if os.path.exists(LEADERBOARD_FILE):
        with open(LEADERBOARD_FILE, 'r') as f:
            try:
                ...
                leaderboard = []
    leaderboard.append({"name": username, "score": settings.score, "time": round(settings.elapsed_time, 2)})
...
def get_username():
    ...
    while not username:
        root = Tk()
        root.withdraw()
        username_input = simpledialog.askstring("Никнейм", "Введите ваш никнейм:")
        root.destroy()
        if username_input:
...
def show_menu():
    ...
    # Добавляем надпись "Dinosaur Game" по центру сверху
...
def show_rules():
    ...
    rules_win.after(10000, rules_win.destroy)

def show_leaders():
    leaders_win = Toplevel()
    leaders_win.title("Таблица лидеров")
    leaders_win.geometry("400x300")
    Label(leaders_win, text="Топ-10 игроков:", font=("Arial", 16)).pack(pady=10)
    if os.path.exists(LEADERBOARD_FILE):
        with open(LEADERBOARD_FILE, 'r') as f:
            try:
                leaderboard = json.load(f)
                for i, entry in enumerate(leaderboard):
                    ...
                    Label(leaders_win, text=f"{i + 1}. {name} - {score} очков за {time_sec}с").pack()
            except json.JSONDecodeError:
                Label(leaders_win, text="Ошибка чтения данных").pack()
    else:
        Label(leaders_win, text="Нет данных").pack()
    Button(leaders_win, text="OK", command=leaders_win.destroy).pack(pady=20)
    leaders_win.after(10000, leaders_win.destroy)
